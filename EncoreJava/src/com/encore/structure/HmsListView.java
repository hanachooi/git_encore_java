...
public class HmsListView {
	...
	public HmsListView(int size) {
		service = new HmsListService(size);
	}
	...
	public void perPrint() {
		...
		List<Person> personLst = service.getAry();
		if(service.getIdx() == 0) {
			System.out.println();
			...
			for(int idx = 0 ; idx < personLst.size() ; idx++) {
				Person per = personLst.get(idx) ;
				if(per == null) {
					break ;
				}
